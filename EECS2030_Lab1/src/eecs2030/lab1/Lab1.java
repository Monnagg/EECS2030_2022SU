...
public class Lab1 {
	...
	public static double pressureAltitude(double pressure) {
		...
		double ft = 145366.45 * (1 - Math.pow((pressure / 1013.25), 0.190284));
		return ft;
	}
	...
	public static boolean enrolStudent(int studentNumber) {
		if (studentNumber / (int) Math.pow(10, 8) == 0) {
	...
	public static String alternatingCaps(String s) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (i % 2 == 0) {
				charOfString[i] = Character.toUpperCase(charOfString[i]);

			} else {
				charOfString[i] = Character.toLowerCase(charOfString[i]);

			}
		}
		String result = String.valueOf(charOfString);
